# iran-war-resolution-full

## テーマ

**調査テーマ**: iran-war-resolution-full

**説明**: イラン戦争収束の鍵：多角的視点からの分析と日本への影響評価

### 前提条件

> イラン戦争収束に関する分析の前提条件・スコープ

- **related_countries**: 米国、イスラエル、イラン、サウジアラビア、ロシア、中国、トルコ、EU諸国、日本など関係国すべてを分析対象とする
- **japan_impact**: 日本への影響（エネルギー安全保障、外交、経済、邦人安全）を必ず分析に含める
- **escalation_risk**: 世界大戦に発展する可能性・シナリオ分析を含める
- **perspective**: 地政学的・軍事的・経済的・外交的な多角的視点

---

## 主要ドキュメント

| ドキュメント | 説明 |
|------------|------|
| [最終レポート（統合レビュー）](./05-integration-review-claude-opus-4.6.md) | 全調査結果を統合した最終結論 |
| [批判的レビュー](./04-critic-review-claude-sonnet-4.6.md) | 論理矛盾・エビデンス不足・バイアスの検証 |
| [情報源一覧](./sources.json) | 全ソースの集約リスト（Source ID・URL・信頼性評価） |
| [関係図（PNG）](./diagram.png) | リサーチ結果の視覚的サマリー |
| [関係図（SVG）](./diagram.svg) | 高解像度ベクター版 |
| [関係図（編集用）](./diagram.drawio) | draw.io で開いて編集可能 |
| [プレゼンスライド（PDF）](./slides.pdf) | プレゼンテーション用スライド |
| [プレゼンスライド（HTML）](./slides.html) | ブラウザで閲覧可能なスライド |

## メタデータ

| 項目 | 値 |
|------|-----|
| **生成方法** | planner |
| **実行日時** | 2026-03-14 19:20:18 |
| **ツール** | AI Research Agent v2.2.0 |
| **手法** | Evidence-based Multi-Model Pipeline |

## パイプライン

| # | 役割 | モデル | 出力 |
|---|------|-------|------|
| 1 | 情報収集 | gemini-3-pro-preview | [情報収集の出力を見る](./01-information-gathering-gemini-3-pro-preview.md) (完了) |
| 2 | 深層分析 | claude-sonnet-4.6 | [深層分析の出力を見る](./02-deep-analysis-claude-sonnet-4.6.md) (完了) |
| 3 | 技術戦略 | gpt-5.3-codex | [技術戦略の出力を見る](./03-technical-strategy-gpt-5.3-codex.md) (完了) |
| 4 | 批判的レビュー | claude-sonnet-4.6 | [批判的レビューの出力を見る](./04-critic-review-claude-sonnet-4.6.md) (完了) |
| 5 | 統合レビュー | claude-opus-4.6 | [統合レビューの出力を見る](./05-integration-review-claude-opus-4.6.md) (完了) |

## 各ステップの要約

### Step 1: 情報収集（gemini-3-pro-preview）

# Iran War Resolution: Multi-perspective Analysis and Impact on Japan (イラン戦争収束：多角的分析と日本への影響)

## 1. Current War Situation and Escalation Risks (戦況の現状とエスカレーションリスク)

### **Claim: War Escalation following Supreme Leader's Death**
イラン最高指導者ハメネイ師の死去（2026年3月1日報道）を受け、イラン国内の動揺と強硬派の台頭により、イスラエルおよび米国に対する報復攻撃が激化している。これに対し、米・イスラエル連合軍は軍事作戦を継続しており、戦況は予断を許さない状況にある。
- **Evidence**: CNBCはハメネイ師の死去後、イランが「より激しく攻撃（lash out harder）」する可能性を報じており、体制の不安定化が軍事的暴走を招いている。
- **Source**: [news_article/tier3] [Iran may 'lash out harder' following Khamenei's death - CNBC](https://www.cnbc.com/2026/03/01/us-israel-attacks-strikes-iran-retaliate-china-russia-allies.html) (2026-03-14)
- **Confidence**: medium (情勢流動的)

### **Claim: Strait of Hormuz Blockade**
イランによるホルムズ海峡の封鎖措置により、世界の石油・LNG輸送の約20%が寸断され、事実上の海上封鎖状態にある。これは世界経済への直接的な攻撃となっており、軍事的緊張の核心である。
- **Evidence**: Middle East Council on Global Affairs および Bloomberg は、海峡の往来がほぼ停止し、エネルギー供給網が麻痺していると報告している。
- **Source**: [industry_report/tier2] [Asia and the Iran Conflict: Energy Vulnerability](https://mecouncil.org/publication/asia-and-the-iran-conflict-energy-vulnerability-and-the-imperative-for-action/) (2026-03-14)
- **Confidence**: high

### **Claim: Risk of Nuclear Proliferation and Great Power Conflict**
ロシアのラブロフ外相は、米・イスラエルの攻撃が地域における核拡散を引き起こすリスクを警告している。ロシアはイランへの情報提供を行っており、中国も物的支援を示唆していることから、代理戦争から大国間の直接対立（第三次世界大戦シナリオ）へ発展する懸念が高まっている。
- **Evidence**: Al Jazeera および Modern Diplomacy によると、中露は米国の軍事介入を強く批判し、外交的・情報的支援を強化している。
- **Source**: [news_article/tier3] [Russia, China raise diplomatic voices against US-Israeli attacks on Iran](https://www.aljazeera.com/news/2026/3/4/russia-china-raise-diplomatic-voices-against-us-israeli-attacks-on-iran) (2026-03-14)

_...（全60行。詳細は[情報収集の全文](./01-information-gathering-gemini-3-pro-preview.md)を参照）_

### Step 2: 深層分析（claude-sonnet-4.6）

前回のステップの出力と提供されたコンテキストを基に、deep-analysis を実施します。

---

# イラン戦争の構造分析：地政学・軍事・エスカレーションシナリオ評価
## Structure Analysis of the Iran Conflict: Geopolitical, Military, and Escalation Scenario Assessment

---

## 1. Structural Overview（戦争の構造的分析）

### **Claim: 「ポスト・ハメネイ」権力空白がもたらす多重的不確実性**

イラン最高指導者ハメネイ師の死去（2026年3月1日）は、単なる指導者交代ではなく、イラン国家体制のレジティマシー（正統性）基盤そのものの喪失を意味する。イスラム共和制の最高意思決定機構である「専門家会議」による後継者選定プロセスは、進行中の軍事衝突下では機能不全に陥るリスクが高い。

- **Evidence**: イラン憲法第111条は最高指導者の死去後に専門家会議が後継を選出する手続きを定めているが、現在の交戦状態では会議の物理的な開催自体が困難。イスラム革命防衛隊（IRGC）が事実上の意思決定権を掌握している可能性が高い。
- **Source**: [official_document/tier1] [Constitution of the Islamic Republic of Iran, Article 111](https://www.constituteproject.org/constitution/Iran_1989) (access_date: unknown) — primary_source: true
- **Confidence**: high

### **Claim: 戦争の三層構造——直接的対立・代理的対立・構造的対立**

_...（全203行。詳細は[深層分析の全文](./02-deep-analysis-claude-sonnet-4.6.md)を参照）_

### Step 3: 技術戦略（gpt-5.3-codex）

# Technical Strategy Assessment for Japan（日本向け技術戦略評価）

## Findings（主要な発見）

### 1) Energy chokepoint exposure（エネルギー要衝への曝露）

- **Claim（主張）**: ホルムズ海峡は、日本の石油・LNG安全保障に対する最大の単一点障害（single point of failure）である。  
- **Evidence（根拠）**: EIAは、ホルムズ経由の石油フローを約2,000万b/d（世界石油消費の約20%）とし、LNGも世界貿易の約20%が通過と示す。  
- **Source（情報源）**:  
  - [official_document/tier1] [Amid regional conflict, the Strait of Hormuz remains critical oil chokepoint (EIA)](https://www.eia.gov/todayinenergy/detail.php?id=65504) (2026-03-14) — primary: true  
  - [official_document/tier1] [In brief analysis: LNG trade through the Strait of Hormuz (EIA)](https://www.eia.gov/todayinenergy/detail.php?id=65584) (2026-03-14) — primary: true  
- **Confidence（確信度）**: high

### 2) Japan’s import concentration risk（日本の輸入集中リスク）

- **Claim（主張）**: 日本の原油調達は中東依存度が極めて高く、紛争長期化時に価格・物流・精製運用の同時ショックを受けやすい。  
- **Evidence（根拠）**: EIA日本国別分析で中東依存の高さが示され、政府エネルギー計画でも安定供給と多角化が中心課題。  
- **Source（情報源）**:  
  - [official_document/tier1] [Country Analysis Brief: Japan (EIA)](https://www.eia.gov/international/content/analysis/countries_long/Japan/japan.pdf) (2026-03-14) — primary: true  
  - [official_document/tier1] [The 7th Strategic Energy Plan Outline (ANRE/METI)](https://www.enecho.meti.go.jp/en/category/others/basic_plan/pdf/7th_outline.pdf) (2026-03-14) — primary: true  

_...（全202行。詳細は[技術戦略の全文](./03-technical-strategy-gpt-5.3-codex.md)を参照）_

### Step 4: 批判的レビュー（claude-sonnet-4.6）

# Critic Review（批判的レビュー）

## Validation Summary（検証サマリー）

| チェック項目 | 件数 | 内訳 |
|---|---|---|
| Claim without Evidence（根拠なしの主張） | **6件** | 下記詳述 |
| Evidence without Source（出典なしの根拠） | **4件** | 下記詳述 |
| Conclusion without Confidence（確信度なしの結論） | **4件** | deep-analysis Section 7の番号付き結論すべて |
| Confidence判定の基準逸脱 | **3件** | Claim①の"high"、シナリオA〜Bの"medium" |

**全体の Validation 結果: PARTIAL**

技術戦略（technical-strategy）は概ね高品質だが、地政学分析（deep-analysis）に構造的な問題が複数存在する。特に確率推定の根拠不明示と、Section 7結論へのConfidence欠落が致命的。

---

## 義務レベル分析（Requirement Level Analysis）

**deep-analysis ステップ**:

_...（全244行。詳細は[批判的レビューの全文](./04-critic-review-claude-sonnet-4.6.md)を参照）_

### Step 5: 統合レビュー（claude-opus-4.6）

Now let me check the existing context file for prior analysis steps:

Now I have sufficient information to compile the comprehensive integration review. Let me write it:

# 統合レビュー

## Executive Summary（総括）

イラン戦争収束の最大のゲームチェンジャーは「IRGC内部の路線対立の帰結」である。ハメネイ師殺害後の権力空白の中で、IRGC強硬派と実利派のどちらが主導権を握るかが、停戦受諾・長期化・さらなるエスカレーションの全シナリオの分岐点となる。外部要因としては、ホルムズ海峡封鎖の長期化がもたらす世界経済への累積ダメージが、米中双方に停戦圧力を生む「経済的疲弊ドライバー」として最も蓋然性の高い収束推進力である。日本はエネルギー備蓄の時間的余裕（約254日分）が残されている間に、①邦人退避の完了、②代替エネルギー調達の多角化、③サイバー防御の緊急強化を同時並行で進める必要がある。核拡散連鎖（サウジアラビアの核武装志向）と偶発的エスカレーションによる大国間衝突リスクは、先行分析で過小評価されており、上方修正が必要である。

## Research Question（調査の問い）

イラン戦争を収束させる最大の「鍵」（ゲームチェンジャー）は何か。最も蓋然性の高い将来シナリオはどれか。日本政府および企業はどのような行動をとるべきか。これらを、批判的レビューで指摘された論理矛盾・エビデンス不足・バイアスを修正した上で統合的に評価する。

## Findings（主要な発見）

### Finding 1: ゲームチェンジャーとしてのIRGC内部路線対立

- **Claim（主張）**: イラン戦争収束の最大の鍵は、IRGC（イスラム革命防衛隊）内部における強硬派と実利派の権力闘争の帰結である。ハメネイ師殺害後、憲法上の後継手続（専門家会議による最高指導者選出）は事実上機能停止しており、暫定指導評議会が権限を行使しているが、実質的な意思決定権はIRGCが掌握している。IRGC強硬派が優勢なら戦争は長期化・エスカレーションし、実利派が主導権を取れば体制存続を優先した停戦交渉への転換が起こりうる。
- **Evidence（根拠）**: ハメネイ師は2026年3月1日の米・イスラエル空爆で死亡。専門家会議は戦時下で招集困難、一部の指名候補は就任拒否。便宜評議会が専門家会議の権限を停止し暫定体制に移行。IRGC内部では「安全保障共和国」化を推進する強硬派と、体制存続のための交渉を模索する実利派が対立。モジタバ・ハメネイ（故指導者の息子）がIRGCとの妥協的象徴として浮上。

_...（全335行。詳細は[統合レビューの全文](./05-integration-review-claude-opus-4.6.md)を参照）_

---

## Trust Summary（信頼性サマリー）

### Validation Result（検証結果）: **PARTIAL**

| 指標 | 値 |
|------|-----|
| 総 Claim（主張）数 | 54 |
| Evidence（根拠）なし | 0 |
| Source（情報源）なし | 11 |
| Confidence（確信度）なし | 0 |

### Confidence Distribution（確信度分布）

| Level（レベル） | 件数 |
|-------|------|
| High（高） | 19 |
| Medium（中） | 30 |
| Low（低） | 5 |

### Source Reliability（情報源の信頼性分布）

| Tier（階層） | 件数 | 説明 |
|------|------|------|
| Tier 1（公式） | 23 | 公式文書・法令・政府機関・標準化団体 |
| Tier 2（高信頼） | 8 | 企業公式・研究機関・学術論文 |
| Tier 3（参考） | 13 | 技術ブログ・専門メディア・業界記事 |
| Tier 4（コミュニティ） | 10 | フォーラム・コミュニティ・SNS |

一次情報（Primary Source）: 23件 / 二次情報: 31件

### Confidence 判定基準（確信度の基準）

| Level（レベル） | 基準 |
|-------|------|
| **High（高）** | 一次情報または複数独立ソースで裏付けあり |
| **Medium（中）** | 信頼できるソースあり、ただし補強不足 |
| **Low（低）** | 推測を含む、または情報不足 |

### Validation Rules（検証ルール）

1. すべての Claim（主張）に Evidence（根拠）が付いていること
2. すべての Evidence（根拠）に Source（情報源）が付いていること
3. すべての結論に Confidence（確信度）が付いていること
4. Critic Review（批判的レビュー）が未解決事項をリスト化していること

### 注意事項

- AI生成のため、重要な意思決定には人間による検証を推奨
- Evidence JSON (`.evidence.json`) で機械的な監査が可能
- Validation 結果 (`.validation.json`) でレポートの品質を定量評価

---

_Generated by [AI Research Agent](https://github.com/ledboots624/ai-research-agent) v2.2.0_
